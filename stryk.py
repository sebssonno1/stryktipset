...
# --- KONFIGURATION ---
ST_PAGE_TITLE = "🐻 Stryk & Europa: Hej Då PM Edition"
...
def fetch_external_odds(api_key):
    ...
    all_odds = {}
    leagues = [
        'soccer_epl', 'soccer_efl_championship', 'soccer_uefa_champs_league', 
        'soccer_uefa_europa_league', 'soccer_spain_la_liga', 'soccer_italy_serie_a', 
        'soccer_germany_bundesliga', 'soccer_france_ligue_one', 'soccer_sweden_allsvenskan',
...
def parse_svenskaspel_paste(text_content):
    ...
    while i < len(lines):
                ...
                for offset in range(1, 5):
                    if i + offset < len(lines):
                        ...
                        if '-' in txt and len(txt) > 3:
                            parts = txt.split('-')
                            current_match = {'Match': int(line), 'Hemmalag': parts[0].strip(), 'Bortalag': parts[1].strip()}
                            break
                        elif txt == '-':
                            current_match = {'Match': int(line), 'Hemmalag': lines[i+offset-1], 'Bortalag': lines[i+offset+1]}
                            break
            except: pass
        if "Svenska folket" in line and current_match:
            try:
                temp_pcts = [int(lines[i+off].replace('%', '')) for off in range(1, 6) if i+off < len(lines) and '%' in lines[i+off]]
                if len(temp_pcts) >= 3:
        ...
        if "Odds" in line and current_match and 'Streck_1' in current_match:
            try:
                temp_odds = [float(lines[i+off].replace(',', '.')) for off in range(1, 6) if i+off < len(lines) and ',' in lines[i+off]]
                if len(temp_odds) >= 3:
...
def suggest_sign_and_status(row):
    ...
    status = "💎 Bra värde" if max_val > 8 else "⚠️ Överstreckad" if max_val < -10 else "Neutral"
...
with st.form("input_form"):
    text_input = st.text_area("Klistra in allt från Svenska Spel här:", height=200)
    submitted = st.form_submit_button("🚀 Analysera", type="primary")

if submitted and text_input:
    matches_data = parse_svenskaspel_paste(text_input)
    if not matches_data:
        st.error("Kunde inte läsa datan. Kopiera allt (Ctrl+A) på Svenska Spels sida.")
    else:
        ...
        with st.expander("Visa detaljerat värde"):
