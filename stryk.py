...
SVENSKA_SPEL_URL = "https://spela.svenskaspel.se/stryktipset?refs=vattendelaren"
...
def fetch_external_odds(api_key):
    ...
    leagues = [
        ...
        'soccer_fa_cup', 'soccer_efl_cup', 'soccer_sweden_allsvenskan', 'soccer_sweden_superettan', 
                ...
                for offset in range(1, 6):
                    if i + offset < len(lines):
                        ...
                        if '-' in txt and len(txt) > 3:
                            parts = txt.split('-')
                            current_match = {'Match': int(line), 'Hemmalag': clean_team_name(parts[0]), 'Bortalag': clean_team_name(parts[1])}
                            break
            except Exception: pass
...
def calculate_probabilities(row):
    o1, ox, o2 = row.get('API_Odds_1', 0), row.get('API_Odds_X', 0), row.get('API_Odds_2', 0)
...
def suggest_sign_and_status(row):
    ...
    if row['Prob_1'] > 55 and val1 > -15: tecken.append('1')
        ...
        status = f"💎 Fynd {values[0][0]}" if values[0][1] > 7 else ("⚠️ Överstreckad" if values[0][1] < -10 else "Neutral")
...
if submitted and text_input:
    ...
    if not matches_data: 
        ...
        for m in matches_data:
            ...
            if external_odds:
                ...
                if score >= MATCH_THRESHOLD: 
                    ...
                    m.update({'API_Odds_1': odds['1'], 'API_Odds_X': odds['X'], 'API_Odds_2': odds['2'], 'Källa': "Odds API", 'Matchat_Lag': match_name})
                    matches_found_in_api += 1
                    matched = True
            if not matched:
                m.update({'Källa': "Saknas", 'API_Odds_1': 0, 'API_Odds_X': 0, 'API_Odds_2': 0})
            final_rows.append(m)

        df = pd.DataFrame(final_rows)
        df[['Prob_1', 'Prob_X', 'Prob_2']] = df.apply(calculate_probabilities, axis=1, result_type='expand')
        df['Val_1'], df['Val_X'], df['Val_2'] = df['Prob_1'] - df['Streck_1'], df['Prob_X'] - df['Streck_X'], df['Prob_2'] - df['Streck_2']
        results = df.apply(suggest_sign_and_status, axis=1, result_type='expand')
        df['Tips'], df['Analys'] = results[0], results[1]
        ...
        st.success(f"Analys klar för {len(df)} matcher.")
        tab1, tab2, tab3 = st.tabs(["💡 Kupong", "📊 Värde", "⚖️ Odds"])
        
        with tab1:
            st.dataframe(df[['Match', 'Match_Rubrik', 'Tips', 'Analys']], hide_index=True, use_container_width=True)
        with tab2:
            st.dataframe(df[['Match', 'Match_Rubrik', 'Val_1', 'Val_X', 'Val_2']], hide_index=True, use_container_width=True)
        with tab3:
            st.dataframe(df[['Match', 'Match_Rubrik', 'API_Odds_1', 'Folk_Odds_1', 'API_Odds_X', 'Folk_Odds_X', 'API_Odds_2', 'Folk_Odds_2']], hide_index=True, use_container_width=True)

# --- SIDFOT ---
st.divider()
st.markdown(f"[Gå till Svenska Spel Stryktipset]({SVENSKA_SPEL_URL})")
